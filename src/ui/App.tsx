...
import type { PriorityTier, ChatMessage, AutoReplyRule, PendingApproval, ChatThreadItem } from './types.js';

export default function AuthenticDarkWhatsAppWebClone() {
  const {
    status,
    user,
    qrDataUrl,
  ...
  return (
    <div className="flex h-screen w-screen bg-[#111b21] text-[#e9edef] font-sans antialiased overflow-hidden select-none">
      ...
      {/* =========================================================================
         ...
         ========================================================================= */}
      <aside className="w-[30%] min-w-[340px] max-w-[450px] bg-[#111b21] border-r border-[#222d34] flex flex-col flex-shrink-0 z-10">
        
        {/* Sidebar Header */}
        <header className="h-[60px] bg-[#202c33] px-4 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            {/* User Profile Avatar */}
            <div className="w-10 h-10 rounded-full bg-[#00a884]/20 border border-[#00a884]/40 flex items-center justify-center font-bold text-[#00a884] text-sm shadow-inner">
              {user?.pushname ? user.pushname.substring(0, 2).toUpperCase() : 'WA'}
            </div>

            {/* MCP Connection Status Indicator */}
            <div className="flex flex-col">
              <span className="text-xs font-semibold text-[#e9edef] leading-tight">
              ...
              >
                <span className={`w-2 h-2 rounded-full ${isConnected ? 'bg-[#00a884] animate-pulse' : isWaitingQR ? 'bg-[#f59e0b]' : 'bg-[#ef4444]'}`} />
                <span className={`font-medium ${isConnected ? 'text-[#00a884]' : isWaitingQR ? 'text-[#f59e0b]' : 'text-[#ef4444]'}`}>
          ...
          <div className="flex items-center gap-1 text-[#aebac1]">
            {isWaitingQR && (
              <button
                ...
                className="p-2 hover:bg-[#374248] rounded-full text-[#f59e0b] hover:text-[#e9edef] transition"
            ...
            <button
              ...
              className="p-2 hover:bg-[#374248] rounded-full hover:text-[#00a884] transition"
            ...
            <button
              ...
              className="p-2 hover:bg-[#374248] rounded-full hover:text-[#e9edef] transition"
        ...
        {/* View Selector Tabs */}
        <div className="px-3 pt-2.5 pb-2 bg-[#111b21] flex items-center gap-1.5 border-b border-[#222d34]">
          <button
        ...
        <div className="px-3 py-2 bg-[#111b21] border-b border-[#222d34]">
          <div className="relative flex items-center bg-[#202c33] rounded-lg px-3 py-1.5">
            <Search size={15} className="text-[#8696a0] mr-3 flex-shrink-0" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search or start new chat"
              className="w-full bg-transparent text-xs text-[#e9edef] placeholder:text-[#8696a0] focus:outline-none"
            />
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex-shrink-0">
                    <div className="w-12 h-12 rounded-full bg-[#202c33] border border-[#2a3942] flex items-center justify-center font-bold text-[#e9edef] text-sm">
                      {chat.senderName.substring(0, 2).toUpperCase()}
                    </div>
                    {chat.isVIP && (
                      <div className="absolute -top-1 -right-1 bg-[#f59e0b] text-[#111b21] p-0.5 rounded-full">
                        <Crown size={10} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between gap-1">
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        {isCrit && (
                          <span className="px-1.5 py-0.2 rounded text-[10px] font-bold bg-[#ef4444] text-white">
                            {chat.priority}
                          </span>
                        )}
                        {chat.isVIP && !isCrit && (
                          <span className="px-1.5 py-0.2 rounded text-[10px] font-bold bg-[#f59e0b] text-[#111b21]">
                            VIP
                          </span>
                        )}
                        {hasApproval && (
                          <span className="px-1.5 py-0.2 rounded text-[10px] font-bold bg-[#3b82f6] text-white animate-pulse">
                            STAGED
                          </span>
                        )}
                      </div>
      ...
      <main className="flex-1 flex flex-col bg-[#0b141a] relative overflow-hidden">
        {activeChat ? (
          <>
            {/* Active Chat Header */}
            <header className="h-[60px] bg-[#202c33] px-4 flex items-center justify-between border-b border-[#222d34] flex-shrink-0 z-10">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#2a3942] flex items-center justify-center font-bold text-[#e9edef] text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {approvalModeActive ? <ShieldAlert size={14} /> : <ShieldCheck size={14} />}
            ...
            {/* Chat Messages Feed with WhatsApp Doodle Wallpaper */}
            <div 
            ...
            >
              ...
              <div className="flex justify-center my-2">
                <div className="bg-[#182229] border border-[#222d34] px-4 py-1.5 rounded-lg text-[11px] text-[#8696a0] flex items-center gap-1.5 shadow-sm max-w-md text-center">
                  <Lock size={12} className="text-[#f59e0b] flex-shrink-0" />
              ...
              {activeChat.messages.map((msg) => {
                ...
                const isCrit = msg.priority === 'CRITICAL' || msg.priority === 'URGENT';
                ...
                return (
                  ...
                  >
                    <div
                      className={`max-w-md md:max-w-lg rounded-xl px-3.5 py-2 text-sm leading-relaxed shadow-sm relative ${
                        isOut 
                    ...
                    {msg.autoReplied && (
                      <span className="text-[10px] text-[#00a884] mt-0.5 flex items-center gap-1">
            ...
            {currentPendingApproval && (
              <div className="mx-4 mb-2 p-3.5 bg-[#202c33]/95 border border-[#f59e0b]/50 rounded-xl shadow-xl backdrop-blur-md animate-in slide-in-from-bottom-2 duration-200">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[10px] font-mono text-[#8696a0]">ID: {currentPendingApproval.id.slice(-6)}</span>
                </div>

                <p className="text-xs text-[#e9edef] bg-[#111b21] p-2.5 rounded-lg border border-[#222d34] mb-3">
                ...
                <div className="flex items-center justify-end gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 bg-[#111b21] hover:bg-[#374248] text-[#ef4444] rounded-lg text-xs font-semibold border border-[#ef4444]/30 transition"
                  >
                    Reject / Edit
                  </button>
                  <button
                    onClick={() => approveMessage(currentPendingApproval.id)}
                    className="px-4 py-1.5 bg-[#00a884] hover:bg-[#00a884]/90 text-[#111b21] rounded-lg text-xs font-bold flex items-center gap-1.5 transition shadow-sm"
                  >
                    <Check size={14} />
            ...
            {/* Bottom Input Bar */}
            <footer className="h-[62px] bg-[#202c33] px-4 flex items-center gap-2 flex-shrink-0">
              <button className="p-2 text-[#8696a0] hover:text-[#e9edef] transition">
                <Smile size={22} />
              </button>
              <button className="p-2 text-[#8696a0] hover:text-[#e9edef] transition">
                <Paperclip size={20} />
              ...
              <form onSubmit={handleSendMessage} className="flex-1 flex items-center gap-2">
                <input
                  ...
                  className="flex-1 bg-[#2a3942] text-sm text-[#e9edef] placeholder:text-[#8696a0] px-4 py-2.5 rounded-lg focus:outline-none"
                />

                <button
                  type="submit"
                  disabled={isSending || !inputText.trim()}
                  className="p-2.5 bg-[#00a884] hover:bg-[#00a884]/90 disabled:opacity-40 text-[#111b21] rounded-full transition shadow-sm flex items-center justify-center flex-shrink-0"
                >
                  <Send size={18} />
                </button>
              </form>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-[#111b21]">
            <div className="w-24 h-24 rounded-full bg-[#202c33] flex items-center justify-center mb-6 text-[#00a884]">
              <MessageSquare size={48} />
            </div>
            <h1 className="text-2xl font-light text-[#e9edef] mb-2">WhatsApp Web MCP</h1>
            <p className="text-sm text-[#8696a0] max-w-md mb-8 leading-relaxed">
      ...
      {/* =========================================================================
         ...
         ========================================================================= */}
      {showDrawer && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs select-none animate-in fade-in">
          <div className="w-full max-w-md h-full bg-[#111b21] border-l border-[#222d34] flex flex-col shadow-2xl animate-in slide-in-from-right duration-200">
            <div className="h-[60px] bg-[#202c33] px-4 flex items-center justify-between border-b border-[#222d34]">
              <div className="flex items-center gap-2">
                <Terminal size={18} className="text-[#00a884]" />
                <h3 className="text-sm font-bold text-[#e9edef]">Live MCP Execution Logs</h3>
              </div>
              <button onClick={() => setShowDrawer(false)} className="p-1 text-[#8696a0] hover:text-[#e9edef]">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2.5 font-mono text-xs">
              {auditLogs.length === 0 ? (
                <div className="p-8 text-center text-[#8696a0]">No tool executions yet</div>
              ) : (
                auditLogs.map((log) => (
                  <div key={log.id} className="p-2.5 rounded-lg bg-[#202c33] border border-[#2a3942] space-y-1">
                    <div className="flex justify-between text-[#8696a0] text-[10px]">
                      <span className="text-[#00a884] font-bold">[{log.type}]</span>
                      <span>{new Date(log.timestamp).toLocaleTimeString()}</span>
                    </div>
                    <p className="text-xs text-[#e9edef]">{log.action}</p>
                  </div>
                ))
              )}
            </div>
      ...
      {showQRModal && qrDataUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-4 animate-in fade-in">
          <div className="bg-[#202c33] border border-[#2a3942] rounded-2xl p-6 max-w-sm w-full text-center space-y-4 shadow-2xl">
            <div className="flex justify-between items-center">
              <h3 className="font-bold text-[#e9edef] text-sm">Pair WhatsApp Mobile App</h3>
